...
import {
    type AlbumArtistDetailArgs,
    type AlbumArtistDetailResponse,
    ...
    type AlbumArtistListResponse,
    type AlbumArtistListArgs,
    ...
    type PlaylistSongListArgs,
    type PlaylistSongListResponse,
    type RemoveFromPlaylistResponse,
    type RemoveFromPlaylistArgs,
    genreListSortMap,
    type ServerInfo,
    type ServerInfoArgs,
    type ShareItemArgs,
    type ShareItemResponse,
    type SimilarSongsArgs,
    type Song,
} from '../types';
import { type VersionInfo, getFeatures, hasFeature } from '@/renderer/api/utils';
import { ServerFeature, type ServerFeatures } from '@/renderer/api/features-types';
...
const getUserList = async (args: UserListArgs): Promise<UserListResponse> => {
    ...
    return {
        items: res.body.data.map((user: any) => ndNormalize.user(user)),
...
const getGenreList = async (args: GenreListArgs): Promise<GenreListResponse> => {
    ...
    return {
        items: res.body.data.map((genre: any) => ndNormalize.genre(genre)),
        startIndex: query.startIndex || 0,
        totalRecordCount: Number(res.body.headers.get('x-total-count') || 0),
    };
};
...
const getAlbumArtistList = async (args: AlbumArtistListArgs): Promise<AlbumArtistListResponse> => {
    ...
    return {
        items: res.body.data.map((albumArtist: any) =>
...
const getAlbumList = async (args: AlbumListArgs): Promise<AlbumListResponse> => {
    ...
    return {
        items: res.body.data.map((album: any) => ndNormalize.album(album, apiClientProps.server)),
        startIndex: query?.startIndex || 0,
        totalRecordCount: Number(res.body.headers.get('x-total-count') || 0),
    };
};
...
const getSongList = async (args: SongListArgs): Promise<SongListResponse> => {
    ...
    return {
        items: res.body.data.map((song: any) =>
...
const getPlaylistList = async (args: PlaylistListArgs): Promise<PlaylistListResponse> => {
    ...
    return {
        items: res.body.data.map((item: any) => ndNormalize.playlist(item, apiClientProps.server)),
        startIndex: query?.startIndex || 0,
        totalRecordCount: Number(res.body.headers.get('x-total-count') || 0),
    };
};
...
const getPlaylistSongList = async (
    ...
    return {
        items: res.body.data.map((item: any) => ndNormalize.song(item, apiClientProps.server, '')),
        startIndex: query?.startIndex || 0,
        totalRecordCount: Number(res.body.headers.get('x-total-count') || 0),
    };
};
...
const getSimilarSongs = async (args: SimilarSongsArgs): Promise<Song[]> => {
    ...
    if (res.status === 200 && res.body.similarSongs?.song) {
        const similar = res.body.similarSongs.song.reduce<Song[]>((acc: any, song: any) => {
    ...
    return fallback.body.data.reduce<Song[]>((acc: any, song: any) => {
        if (song.id !== query.songId) {
