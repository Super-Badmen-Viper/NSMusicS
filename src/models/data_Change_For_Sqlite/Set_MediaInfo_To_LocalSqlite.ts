export class Set_MediaInfo_To_LocalSqlite {
    public Set_MediaInfo_To_Favorite(id: string, value: boolean) {
        ...
        if (!existingRecord) {
            ...
            const updateStmt = db.prepare(`UPDATE annotation SET starred = ? WHERE item_id = ? AND item_type = 'media_file'`);
    ...
    public Set_MediaInfo_To_Rating(id: any, value: number) {
        ...
        if (!existingRecord) {
            ...
            const updateStmt = db.prepare(`UPDATE annotation SET rating = ? WHERE item_id = ? AND item_type = 'media_file'`);
