...
export class Set_ArtistInfo_To_LocalSqlite {
    ...
    public Set_ArtistInfo_To_Rating(id: any, value: number) {
        ...
        if (!existingRecord) {
            ...
            db.prepare(`UPDATE ${store_sqlite_table_info.annotation} SET rating = ? WHERE item_id = ? AND item_type = 'artist'`)
            .run(value, id);
        }
    ...
    public Set_ArtistInfo_To_PlayCount_of_Artist(item_id: any) {
        ...
        if (!existingRecord) {
            ...
            db.prepare(`UPDATE ${store_sqlite_table_info.annotation} SET play_count = ?, play_date = ? WHERE item_id = ? AND item_type = 'artist'`)
