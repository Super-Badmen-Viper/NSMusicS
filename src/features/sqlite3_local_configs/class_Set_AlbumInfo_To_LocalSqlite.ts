...
export class Set_AlbumInfo_To_LocalSqlite {
    ...
    public Set_AlbumInfo_To_Rating(id: any, value: number) {
        ...
        if (!existingRecord) {
            ...
            db.prepare(`UPDATE ${store_model_check_of_sqlite_tablename.annotation} SET rating = ? WHERE item_id = ? AND item_type = 'album'`)
            .run(value, id);
        }
    ...
    public Set_AlbumInfo_To_PlayCount_of_Album(item_id: any) {
        ...
        if (!existingRecord) {
            ...
            db.prepare(`UPDATE ${store_model_check_of_sqlite_tablename.annotation} SET play_count = ?, play_date = ? WHERE item_id = ? AND item_type = 'album'`)
