        ...
        try{
            ...
            if(getArtists_ALL["subsonic-response"]["status"] === 'ok') {
                ...
                artist_list.forEach((artist) => {
                    /// getMusicDirectory_id
                    const getMusicDirectory_id = browsing_ApiService_of_ND.getMusicDirectory_id(username, token, salt, artist.id)
                    if(getMusicDirectory_id["subsonic-response"]["status"] === 'ok') {
                        const song_list_of_this_artist = getMusicDirectory_id["subsonic-response"]["directory"]["child"]
