...
export const store_app_configs_logic_load = reactive({
    ...
    async load_app_config() {
        ...
        try {
            ...
            // store_app_configs_info.player_view_left_menu_collapsed = '' + system_Configs_Read.app_Configs.value['player_view_left_menu_collapsed'] === 'true'
            store_app_configs_info.player_view_left_menu_collapsed = true
        ...
        store_app_configs_info.player_view_left_menu_select_activeKey = '' + system_Configs_Read.app_Configs.value['player_view_left_menu_select_activeKey']
        ...
        if(store_router_data_info.router_name === ''){
            store_app_configs_info.player_view_left_menu_select_activeKey = 'View_Menu_AppSetting'
    ...
    handleUpdate_selectd_props_app_sidebar_Value(value: number[]){
        let allMenuOptions = store_app_configs_info.player_view_menuOptions;
        ...
        if(removeFlags[4] && removeFlags[5] && removeFlags[6] && removeFlags[7])
            ...
            removeFlags[8] = false;
        let player_view_menuOptions = allMenuOptions.filter((option, index) => {
            return !removeFlags[index];
        });
        store_app_configs_info.player_view_menuOptions = player_view_menuOptions
    }
});