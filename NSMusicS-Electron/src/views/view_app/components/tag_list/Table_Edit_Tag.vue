  ...
  try {
    if (isElectron) {
      if (store_server_user_model.model_server_type_of_local) {
        if (playerTagModifyStore.player_show_tag_kind === 'media') {
          const _path = playerTagModifyStore.player_current_media_path
          ...
          if (local_file) {
            const artist = playerTagModifyStore.player_current_media_tag.artist
            const albumArtists = playerTagModifyStore.player_current_media_tag.albumArtists
            const genres = playerTagModifyStore.player_current_media_tag.genres
            //
            ...
            //
            playerTagModifyStore.player_current_media_tag.artist = array_artist
            playerTagModifyStore.player_current_media_tag.albumArtists = array_albumArtists
            playerTagModifyStore.player_current_media_tag.genres = array_genres
            //
            const _tag = JSON.parse(JSON.stringify(playerTagModifyStore.player_current_media_tag))
          ...
          ///
          const save_tag = JSON.parse(JSON.stringify(playerTagModifyStore.player_current_media_tag))
          const item: Media_File | undefined = pageMediaStore.media_Files_temporary.find(
            (mediaFile: Media_File) =>
              mediaFile.path === playerTagModifyStore.player_current_media_path
          )
        ...
        } else if (playerTagModifyStore.player_show_tag_kind === 'album') {
          const _tag = JSON.parse(JSON.stringify(playerTagModifyStore.player_current_album_tag))
          ...
          db.close()
          ///
          const save_tag = JSON.parse(JSON.stringify(playerTagModifyStore.player_current_album_tag))
          const item: Album | undefined = pageAlbumStore.album_Files_temporary.find(
            (album: Album) => album.id === playerTagModifyStore.player_current_album_id
          )
          ...
          item.genre = Array.isArray(save_tag.genres)
            ? save_tag.genres.join('、') || ''
            : save_tag.genres || ''
        } else if (playerTagModifyStore.player_show_tag_kind === 'artist') {
          const _tag = JSON.parse(JSON.stringify(playerTagModifyStore.player_current_artist_tag))
          const artistStr = Array.isArray(_tag.artist) ? _tag.artist.join('、') : _tag.artist || ''
          const genresStr = Array.isArray(_tag.genres) ? _tag.genres.join('、') : _tag.genres || ''
          //
          const db = require('better-sqlite3')(store_system_configs_info.navidrome_db)
          ...
          if (store_server_user_model.model_server_type_of_local_server_download) {
            ...
            const sql_artist = `UPDATE artist
                 SET name = ?
                 WHERE id = ?`
            db.prepare(sql_artist).run(artistStr, playerTagModifyStore.player_current_artist_id)
          }
          //
          db.close()
          ///
          const save_tag = JSON.parse(
            JSON.stringify(playerTagModifyStore.player_current_artist_tag)
          )
          const item: Artist | undefined = pageArtistStore.artist_Files_temporary.find(
            (artist: Artist) => artist.id === playerTagModifyStore.player_current_artist_id
          )
...
<template>
  <n-space vertical :size="12">
    <n-table
      v-if="player_show_tag_kind === 'media'"
      style="
        width: 630px;
    ...
    >
      <tr>
        ...
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_media_tag.title }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder=""
          v-model:value="player_current_media_tag.title"
        />
      </tr>
      <tr>
        <td>{{ $t('filter.path') }}</td>
        <td>{{ player_current_media_tag.path }}</td>
      </tr>
      <tr
        v-if="
      ...
      >
        ...
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_media_tag.albumArtists }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          ...
          v-model:value="player_current_media_tag.albumArtists"
        />
      </tr>
      <tr>
        <td>{{ $t('entity.artist_other') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_media_tag.artist }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder="artist_1、artist_2"
          v-model:value="player_current_media_tag.artist"
        />
      </tr>
      <tr>
        <td>{{ $t('entity.album_other') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_media_tag.album }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder=""
          v-model:value="player_current_media_tag.album"
        />
      </tr>
      <tr>
        <td>{{ $t('filter.disc') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_media_tag.discCount }}
        </td>
        <n-input-number
          v-else
          style="width: 500px"
          ...
          placeholder=""
          :min="0"
          v-model:value="player_current_media_tag.discCount"
        />
      </tr>
      <tr>
        <td>{{ $t('common.trackNumber') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_media_tag.trackCount }}
        </td>
        <n-input-number
          v-else
          style="width: 500px"
          ...
          placeholder=""
          :min="0"
          v-model:value="player_current_media_tag.trackCount"
        />
      </tr>
      <tr>
        <td>{{ $t('common.year') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_media_tag.year }}
        </td>
        <n-input-number
          v-else
          style="width: 500px"
          ...
          placeholder=""
          :min="0"
          v-model:value="player_current_media_tag.year"
        />
      </tr>
      <tr>
        <td>{{ $t('entity.genre_other') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_media_tag.genres }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder=""
          v-model:value="player_current_media_tag.genres"
        />
      </tr>
      <tr>
        <td>{{ $t('common.duration') }}</td>
        <td>{{ player_current_media_tag.duration }}</td>
      </tr>
      <tr>
        <td>{{ $t('filter.isCompilation') }}</td>
        <td>{{ player_current_media_tag.isCompilation }}</td>
      </tr>
      <tr>
        <td>{{ $t('common.codec') }}</td>
        <td>{{ player_current_media_tag.codec }}</td>
      </tr>
      <tr>
        <td>{{ $t('common.bitrate') }}</td>
        <td>{{ player_current_media_tag.audioBitrate }}</td>
      </tr>
      <tr>
        <td>{{ $t('common.channel_other') }}</td>
        <td>{{ player_current_media_tag.audioChannels }}</td>
      </tr>
      <tr>
        <td>{{ $t('common.size') }}</td>
        <td>{{ player_current_media_tag.sizeOnDisk }}</td>
      </tr>
      <tr>
        <td>{{ $t('common.favorite') }}</td>
        <td>{{ player_current_media_starred }}</td>
      </tr>
      <tr>
        <td>{{ $t('filter.playCount') }}</td>
        <td>{{ player_current_media_playCount }}</td>
      </tr>
      <tr>
        <td>{{ $t('filter.lastPlayed') }}</td>
        <td>{{ player_current_media_playDate }}</td>
      </tr>
      <tr v-if="false">
      ...
      <tr>
        ...
        <td>{{ player_current_media_tag.albumPeak }}</td>
      </tr>
      <tr>
        <td>{{ $t('common.trackPeak') }}</td>
        <td>{{ player_current_media_tag.trackPeak }}</td>
      </tr>
      <tr>
        <td>{{ $t('filter.comment') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_media_tag.comment }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder=""
          v-model:value="player_current_media_tag.comment"
        />
      </tr>
      <tr v-if="!store_server_user_model.model_server_type_of_web">
        ...
        <n-input
          ...
          placeholder=""
          type="textarea"
          v-model:value="player_current_media_tag.lyrics"
        />
      </tr>
    </n-table>
    <n-table
      v-if="player_show_tag_kind === 'album'"
      style="
        width: 630px;
        --n-td-color-modal: transparent;
        --n-border-color-modal: #ffffff80;
    ...
    >
      <tr>
        <td style="width: 160px">{{ $t('filter.title') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_album_tag.title }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder=""
          v-model:value="player_current_album_tag.title"
        />
      </tr>
      <tr
        v-if="
      ...
      >
        <td>{{ $t('entity.albumArtist_other') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_album_tag.albumArtists }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder="artist_1、artist_2"
          v-model:value="player_current_album_tag.albumArtists"
        />
      </tr>
      <tr>
        <td>{{ $t('entity.artist_other') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_album_tag.artist }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder="artist_1、artist_2"
          v-model:value="player_current_album_tag.artist"
        />
      </tr>
      <tr>
        <td>{{ $t('common.year') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_album_tag.year }}
        </td>
        <n-input-number
          v-else
          style="width: 500px"
          ...
          placeholder=""
          :min="0"
          v-model:value="player_current_album_tag.year"
        />
      </tr>
      <tr>
        <td>{{ $t('entity.genre_other') }}</td>
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_album_tag.genres }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder=""
          v-model:value="player_current_album_tag.genres"
        />
      </tr>
      <tr>
        <td>{{ $t('common.duration') }}</td>
        <td>{{ player_current_album_tag.duration }}</td>
      </tr>
      <tr>
        <td>{{ $t('filter.isCompilation') }}</td>
        <td>{{ player_current_album_tag.isCompilation }}</td>
      </tr>
      <tr v-if="player_show_tag_kind === 'album'">
        <td>{{ $t('filter.songCount') }}</td>
        <td>{{ player_current_album_tag.songCount }}</td>
      </tr>
    </n-table>
    <n-table
      v-if="player_show_tag_kind === 'artist'"
      style="
        width: 630px;
        --n-td-color-modal: transparent;
        --n-border-color-modal: #ffffff80;
    ...
    >
      <tr>
        ...
        <td v-if="store_server_user_model.model_server_type_of_web">
          {{ player_current_artist_tag.artist }}
        </td>
        <n-input
          v-else
          style="width: 500px"
          clearable
          placeholder="artist_1、artist_2"
          v-model:value="player_current_artist_tag.artist"
        />
      </tr>
      <tr>
        <td>{{ $t('entity.genre_other') }}</td>
        <td>{{ player_current_artist_tag.genres }}</td>
      </tr>
    </n-table>
