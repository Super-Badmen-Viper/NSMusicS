...
const { metrics, getRadarData, increase, isMax, isMin } = []
  ...
  <v-example id="radar" title="Radar chart" desc="(with Pinia integration)">
    ...
    <template #extra>
      <p class="actions">
        ...
        <button @click="increase(metricIndex, 1)" :disabled="isMax(metricIndex)">Increase</button>
        <button @click="increase(metricIndex, -1)" :disabled="isMin(metricIndex)">Decrease</button>
      </p>
    </template>
  </v-example>
</template>
