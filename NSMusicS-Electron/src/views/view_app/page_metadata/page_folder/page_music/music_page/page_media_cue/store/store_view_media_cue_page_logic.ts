...
export const store_view_media_cue_page_logic = reactive({
    ...
    set_media_Files_selected(value: Media_File) {
        if (value.selected) {
            store_view_media_cue_page_info.media_Files_temporary.forEach((item: any, index: number) => {
                if (item.id === value.id) {
                    store_view_media_cue_page_info.media_Files_temporary[index].selected = true;
                }
            });
            store_view_media_cue_page_info.media_Files_selected.push(value)
            console.log('media_Files_selected：'+value.path+'  '+value.selected)
        } else {
            store_view_media_cue_page_info.media_Files_temporary.forEach((item: any, index: number) => {
                if (item.id === value.id) {
                    store_view_media_cue_page_info.media_Files_temporary[index].selected = false;
                }
            });
            store_view_media_cue_page_info.media_Files_selected = store_view_media_cue_page_info.media_Files_selected.filter((item: any) => item.id !== value.id);
            console.log('media_Files_selected：'+value.path+'  '+value.selected)
        }
    },
    set_media_Files_selected_all(value: boolean) {
        store_view_media_cue_page_info.media_Files_temporary.forEach((item: any, index: number) => {
            store_view_media_cue_page_info.media_Files_temporary[index].selected = value;
        });
        if (value) {
            store_view_media_cue_page_info.media_Files_selected = store_view_media_cue_page_info.media_Files_temporary.slice();
        } else {
            store_view_media_cue_page_info.media_Files_selected = [];
        }
        ...
        if (store_server_user_model.model_select === 'server') {
            await store_server_data_set_playlistInfo.Set_Selected_MediaInfo_Delete_Selected_Playlist(
                store_server_users.server_select_kind === 'emby' ?
                    store_view_media_cue_page_info.media_Files_selected.map((file: any) => file.order_title) :
                    store_view_media_cue_page_info.media_Files_selected.map((file: any) => file.id),
                value
            )
        }
    ...
    async get_page_songlists_keyword(newValue: any){
        ...
        if (newValue.indexOf('accurate_search') > 0) {
            ...
            if (newValue.indexOf('__title__') > 0) {
                ...
                store_view_media_cue_page_logic.page_songlists_keywordFilter = `WHERE title LIKE '${newValue}'`
            } else if (newValue.indexOf('__artist__') > 0) {
                newValue = newValue.replace('__artist__', '');
                store_view_media_cue_page_logic.page_songlists_keywordFilter = `WHERE artist LIKE '${newValue}'`
            } else if (newValue.indexOf('__album__') > 0) {
                newValue = newValue.replace('__album__', '');
                store_view_media_cue_page_logic.page_songlists_keywordFilter = `WHERE album_id = '${newValue}'`
            }
        }else{
            store_view_media_cue_page_logic.page_songlists_keywordFilter = newValue.length > 0 ?
                `WHERE title LIKE '%${newValue}%' OR artist LIKE '%${newValue}%' OR album LIKE '%${newValue}%'` :
                '';
        }
        store_view_media_cue_page_logic.page_songlists_keyword_reset = true;
        console.log('page_songlists_keyword:' + newValue)

        store_router_data_info.router.push('media_cue')
...
watch(() => store_view_media_cue_page_logic.page_songlists_options_Sort_key, async (newValue) => {
    if (newValue != null && store_view_media_cue_page_logic.list_options_Hand_Sort) {
...
watch(() => store_view_media_cue_page_logic.list_data_StartUpdate, async (newValue) => {
    if(newValue) {
        store_view_media_cue_page_logic.page_songlists_keyword = '';
        if (store_view_media_cue_page_logic.list_selected_Hand_click) {
            store_view_media_cue_page_logic.page_songlists_keywordFilter = ""
        ...
        store_view_media_cue_page_logic.list_data_StartUpdate = false
        console.log("store_view_media_cue_page_logic.list_data_StartUpdate")
    }
});
watch(() => store_view_media_cue_page_logic.page_songlists_filter_year, async (newValue) => {
    store_view_media_cue_page_logic.page_songlists_filter_model = newValue !== 0
    await store_app_configs_logic_save.save_system_config_of_App_Configs()
    store_view_media_cue_page_logic.page_songlists_keywordFilter = ""
    store_view_media_cue_page_logic.list_selected_Hand_click = false
    await store_general_fetch_media_cue_list.fetchData_Media()
});
watch(() => store_view_media_cue_page_logic.page_songlists_filter_path_folder, async (newValue) => {
    store_view_media_cue_page_logic.page_songlists_filter_model = newValue !== ''
    await store_app_configs_logic_save.save_system_config_of_App_Configs()
    store_view_media_cue_page_logic.page_songlists_keywordFilter = ""
    store_view_media_cue_page_logic.list_selected_Hand_click = false
    await store_general_fetch_media_cue_list.fetchData_Media()
