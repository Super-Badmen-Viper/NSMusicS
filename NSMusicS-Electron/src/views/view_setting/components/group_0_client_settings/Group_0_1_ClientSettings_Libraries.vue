...
import { BareMetalServer, Add, Close } from '@vicons/carbon'
...
onMounted(() => {
  ...
  else Type_Server_Model_Open_Value.value = Type_Server_Model_Open_Option.value[1].value
})
const server_set_of_addUser_of_type = ref(Type_Server_Kinds[2].value)
let unwatch_server_set_of_addUser_of_type = watch(
...
async function update_server_apikey_user_option(data) {
  if (store_server_user_model.server_login_model_of_apikey) {
    store_server_user_model.authorization_of_Je =
      data.apikey.length > 0 ? data.apikey : server_set_of_addUser_of_apikey.value
    // load User
    const userService = new Users_ApiService_of_Je(
      data.url.length > 0 ? data.url : server_set_of_addUser_of_url.value
    )
...
async function update_server_config_of_current_user_of_sqlite(value: any, select_change: any) {
  if (select_change) {
    try {
      ...
      const result =
        await store_server_data_select_logic.update_server_config_of_current_user_of_sqlite(
          value,
          user_config?.type
        )
      if (result) {
...
import { Library_ApiService_of_Je } from '@/data/data_access/servers_configs/jellyfin_api/services_web/Library/index_service'
...
<template>
  <n-space style="overflow-y: auto; overflow-x: hidden; margin-top: 9px">
    <!-- 媒体库管理 -->
    <n-space vertical>
      ...
      <n-space vertical>
        <n-steps vertical style="margin-left: 2px; margin-top: 2px" :status="currentStatus">
          <n-step v-if="isElectron" :title="model_server_step_1">
                        ...
                        <n-modal v-model:show="item.show">
                          <n-card style="width: 450px; border-radius: 4px">
                            <n-space vertical size="large" style="width: 400px">
                              ...
                              <n-form style="margin-top: -12px">
                                <n-space vertical size="small" style="margin-bottom: 10px">
                                  ...
                                  <n-input
                                    clearable
                                    size="small"
                                    v-model:value="item.server_name"
                                    placeholder=""
                                  />
                                </n-space>
                                <n-space vertical size="small" style="margin-bottom: 10px">
                              ...
                              <n-form
                                v-if="store_server_user_model.server_login_model_of_apikey"
                              ...
                              <n-form v-else style="margin-top: -12px">
                                <n-space vertical size="small" style="margin-bottom: 10px">
                                  <span v-if="item.type != 'ninesong'">{{
                                    $t('form.addServer.input_username')
                                  }}</span>
                              ...
                              <n-space justify="end">
                                <n-button
                                  ...
                                  @click="
                                    () => {
                                      item.show = false
                                      update_server_deleteUser(item.id, item.type)
                                    }
                                  "
                                >
                                  {{ $t('common.delete') }}
            ...
            <div>
              <n-space vertical>
                <n-space vertical>
                  <n-space vertical>
                    <span style="font-size: 16px; font-weight: 600">{{
                      $t('nsmusics.view_page.modelServer_type_1') +
                      ' - ' +
                      $t('Suggestions') +
                      $t('ButtonActivate')
                    }}</span>
                    <div style="margin-top: -10px">
                      <span style="font-size: 12px">{{
                        $t('nsmusics.view_page.modelServer_type_1_explain')
                      }}</span>
                    </div>
                  </n-space>
                  <n-switch
                ...
                <n-space vertical>
                  <n-space vertical>
                    <span style="font-size: 16px; font-weight: 600">{{
                      $t('nsmusics.view_page.modelServer_type_2') +
                      ' - ' +
                      $t('Experimental') +
                      ' - ' +
                      $t('DisablePlugin')
                    ...
                    <div style="margin-top: -10px">
                      <span style="font-size: 12px">{{
                        $t('nsmusics.view_page.modelServer_type_2_explain')
                      }}</span>
                    </div>
                  </n-space>
                  <n-switch
                ...
                <n-space vertical v-if="store_server_user_model.model_server_type_of_local">
                  <n-space vertical style="width: 320px">
                    <span style="font-size: 16px; font-weight: 600">
                      {{
                        $t('nsmusics.view_page.modelServer_type_2') +
                        ' | ' +
                        $t('nsmusics.view_page.selectServer')
                      }}
                    </span>
                    <div style="margin-top: -10px">
                      <span style="font-size: 12px">
                  ...
                  <n-space align="end">
                    <n-progress
                      type="line"
                      style="width: 207px; margin-bottom: 8px"
            ...
            <div>
              <n-space vertical>
                <n-select
                  ...
                  style="width: 220px; margin-top: 6px"
                  @update:value="
                    (value: number) => update_server_config_of_current_user_of_sqlite(value, true)
                  "
                />
              </n-space>
            </div>
              ...
              <n-space vertical>
                <n-space justify="space-between" align="center">
                  <n-space vertical style="width: 520px">
                    ...
                    <DynamicScroller
                      v-if="Type_Server_Model_Open_Value === 'local'"
                      class="table"
                      ref="scrollbar"
                      style="overflow: auto; width: 580px; height: 130px; margin-top: 6px"
    ...
    <n-modal v-model:show="Type_Server_Add">
      <n-card style="width: 450px; border-radius: 4px">
        <n-space vertical size="large" style="width: 400px">
          ...
          <n-form inline>
            <n-space vertical style="width: 150px; margin-bottom: 10px">
              ...
              <n-input
                clearable
                placeholder="any"
                v-model:value="server_set_of_addUser_of_servername"
              />
            </n-space>
            <n-space vertical style="width: 250px; margin-bottom: 10px">
              ...
              <n-input-group>
                <n-input
                  ...
                  v-model:value="server_set_of_addUser_of_url"
                />
              </n-input-group>
            </n-space>
          </n-form>
          ...
          <n-form v-else style="margin-top: -12px">
            <n-space vertical style="margin-bottom: 10px">
              <span v-if="server_set_of_addUser_of_type != 'ninesong'">{{
                $t('form.addServer.input_username')
              }}</span>
              <span v-else>{{ $t('nsmusics.server_page.server_email') }}</span>
            ...
            <n-space vertical style="margin-bottom: 10px">
              ...
              <n-input
                ...
                show-password-on="click"
                placeholder=""
                v-model:value="server_set_of_addUser_of_password"
              />
            </n-space>
          </n-form>
          <n-space justify="end">
