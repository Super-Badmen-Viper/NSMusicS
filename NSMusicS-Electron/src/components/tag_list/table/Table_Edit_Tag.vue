  ...
  try {
    if (store_server_user_model.model_server_type_of_local) {
      if (store_player_tag_modify.player_show_tag_kind === 'media') {
        ...
        ///
        const save_tag = JSON.parse(JSON.stringify(store_player_tag_modify.player_current_media_tag));
        ...
        item.artist = Array.isArray(save_tag.artist) ? save_tag.artist.join('、') || '' : save_tag.artist || ''
        ...
        item.genre = Array.isArray(save_tag.genres) ? save_tag.genres.join('、') || '' : save_tag.genres || ''
        ...
        const nameStr =
            Array.isArray(_tag.title) ?
                _tag.title.join('、') : _tag.title || '';
        const artistStr =
            Array.isArray(_tag.artist) ?
                _tag.artist.join('、') : _tag.artist || '';
        const genresStr =
        ...
        db.close()
        ///
        const save_tag = JSON.parse(JSON.stringify(store_player_tag_modify.player_current_album_tag));
        const item: Album | undefined =
            store_view_album_page_info.album_Files_temporary.find(
                (album: Album) =>
                    ...
                    ===
                    store_player_tag_modify.player_current_album_id);
        item.name = Array.isArray(save_tag.title) ? save_tag.title.join('、') || '' : save_tag.title || ''
        item.artist = Array.isArray(save_tag.artist) ? save_tag.artist.join('、') || '' : save_tag.artist || ''
        ...
        db.close()
        ///
        const save_tag = JSON.parse(JSON.stringify(store_player_tag_modify.player_current_artist_tag));
        const item: Artist | undefined =
            store_view_artist_page_info.artist_Files_temporary.find(
                (artist: Artist) =>
                    ...
                    ===
                    store_player_tag_modify.player_current_artist_id);
        item.name = Array.isArray(save_tag.artist) ? save_tag.artist.join('、') || '' : save_tag.artist || ''
      }
