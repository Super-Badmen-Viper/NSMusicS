...
export class Audio_ApiService_of_Je extends Jellyfin_Api_Services_Web {
  ...
  public async getAudio_lyrics_id_of_Em(
    ...
    let response1
    try {
      response1 = await axios.get(`${baseUrl}/emby/Users/${userId}/Items/${itemId}`, {
        params: {
          fields: 'ShareLevel',
    ...
    try {
      ...
      const response2 = await axios.get(lyricUrl, {
        headers: {
          Authorization: `MediaBrowser Token="${token}", Client="NSMusicS", Device="Desktop Client", DeviceId="NineSong", Version="2.0.1"`,
        },
        timeout: 8000,
        responseType: 'json',
      })
