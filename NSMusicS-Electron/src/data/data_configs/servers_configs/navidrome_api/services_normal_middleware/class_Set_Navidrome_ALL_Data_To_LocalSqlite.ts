...
import { Set_AlbumInfo_To_LocalSqlite } from '@/data/data_config_app/app_repository/class_Set_AlbumInfo_To_LocalSqlite'
import { Set_MediaInfo_To_LocalSqlite } from '@/data/data_config_app/app_repository/class_Set_MediaInfo_To_LocalSqlite'
...
export class Set_Navidrome_ALL_Data_To_LocalSqlite {
  ...
  private insertData(db: any, table: any, data_old: any) {
    ...
    const data = { ...data_old }
    if (table === 'server_artist' && data.hasOwnProperty('albums')) delete data.albums
    if (table === 'server_album' && data.hasOwnProperty('media')) delete data.media
      ...
      const artistPromises = list.flatMap((artist_list: any) =>
        artist_list.artist.map(async (artist: any) => {
          ...
          if (!getMusicDirectory_id) {
            getMusicDirectory_id = await browsing_ApiService_of_ND.getMusicDirectory_id(
              username,
              token,
              salt,
              artist.id
          ...
          const song_list_of_this_album =
            getMusicDirectory_id['subsonic-response']['directory']['child']
          const artist_song_count =
            getMusicDirectory_id['subsonic-response']['directory']['songCount']
          const albumPromises = song_list_of_this_album.map(async (album: any) => {
            ...
            try {
              if (
                ...
                db.prepare(
                  `INSERT INTO ${store_server_user_model.annotation}
                                                (ann_id, item_id, item_type, play_count, play_date)
                                            VALUES (?, ?, ?, ?, ?)`
                ).run(
            ...
            if (!getAlbum_id) {
              getAlbum_id = await browsing_ApiService_of_ND.getAlbum(
                username,
                token,
                salt,
                album.id
          ...
          const albumMedias = await Promise.all(albumPromises)
          return albumMedias.flat()
        })
      )
      ...
      try {
        ...
        resultArray.forEach((music) => {
          ...
          if (artist) {
            ...
            store_local_data_set_artistInfo.Set_ArtistInfo_To_Rating(
              artist.id,
              artist.userRating || 0
            )
          }
          ...
          music.artist.albums.forEach((album: any) => {
            ...
            if (albumData) {
              ...
              store_local_data_set_albumInfo.Set_AlbumInfo_To_Favorite(
                albumData.id,
                !albumData.starred
              )
              store_local_data_set_albumInfo.Set_AlbumInfo_To_Rating(
                albumData.id,
            ...
            album.media.forEach((song: any) => {
              ...
              if (songData) {
                ...
                store_local_data_set_mediaInfo.Set_MediaInfo_To_Favorite(
                  songData.id,
                  !songData.starred
                )
                store_local_data_set_mediaInfo.Set_MediaInfo_To_Rating(
                  songData.id,
    ...
    if (isElectron) {
      ...
      if (playlists != null) {
        for (const playlist of playlists) {
          ...
          const getPlaylist_id = await playlists_ApiService_of_ND.getPlaylist_id(
            username,
            token,
            salt,
            playlist.id
          ...
          playlMedias = Array.isArray(getPlaylist_id['subsonic-response']['playlist']['entry'])
