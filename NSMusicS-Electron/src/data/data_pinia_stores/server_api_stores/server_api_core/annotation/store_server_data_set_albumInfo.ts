...
export const useServerDataSetAlbumInfoStore = defineStore('serverDataSetAlbumInfo', () => {
  // 获取其他store
   ...
   * 设置专辑为收藏/取消收藏
   */
  async function Set_AlbumInfo_To_Favorite_Server(item_id: string, value: boolean) {
    if (
   ...
   * 设置专辑评分
   */
  async function Set_AlbumInfo_To_Rating_Server(item_id: any, value: any) {
    if (
      serverUserModelStore.model_server_type_of_local ||
      (serverUsersStore.server_select_kind === 'navidrome' &&
        serverUserModelStore.model_server_type_of_web)
   ...
   * 设置专辑播放次数
   */
  async function Set_AlbumInfo_To_PlayCount_of_Album_Server(item_id: any) {
    if (
      serverUserModelStore.model_server_type_of_local ||
      (serverUsersStore.server_select_kind === 'navidrome' &&
        serverUserModelStore.model_server_type_of_web)
   ...
   * 设置专辑播放完成次数
   */
  async function Set_AlbumInfo_To_PlayCompleteCount_of_Album_Server(item_id: any) {
    if (
      serverUserModelStore.model_server_type_of_web &&
      serverUsersStore.server_select_kind === 'ninesong'
    ) {
  ...
  // 暴露方法
  return {
    Set_AlbumInfo_To_Favorite_Server,
    Set_AlbumInfo_To_Rating_Server,
    Set_AlbumInfo_To_PlayCount_of_Album_Server,
    Set_AlbumInfo_To_PlayCompleteCount_of_Album_Server
  }
})