...
import { useGeneralFetchAlbumListStore } from '@/data/data_stores/server_api_stores/server_api_core/page/page_album/store_general_fetch_album_list'
...
export const useGeneralFetchMediaCueListStore = defineStore('generalFetchMediaCueList', () => {
  ...
  // 监听播放列表显示状态
  watch(
    () => usePlaylistStore().playlist_show,
    async (newValue) => {
      if (newValue) {
        _load_model.value = 'play'
        const index = usePlaylistStore().playlist_MediaFiles_temporary.length / 30
        if (index > 0) {
