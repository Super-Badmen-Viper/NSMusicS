...
/**
 * 播放列表模型管理 store
 * 提供播放列表的创建、更新、删除和获取功能
 */
export const useGeneralModelPlayerListStore = defineStore('generalModelPlayerList', () => {
  // 如果需要状态，可以在这里定义
   ...
   * 根据服务器类型获取不同的播放列表数据
   */
  async function get_playlists_info() {
   ...
   * 更新媒体文件的播放列表
   */
  async function get_playlist_tracks_temporary_update_media_file() {
    store_playlist_list_info.playlist_names_ALLLists = []
    ...
    if (
      ...
      get_PlaylistInfo_From_LocalSqlite.Get_Playlist().forEach((item: any) => {
        store_playlist_list_info.playlist_names_ALLLists.push({
          label: item.name,
          value: item.id,
        })
  ...
  return {
    // 方法暴露
    get_playlists_info,
    get_playlist_tracks_temporary_add,
    get_playlist_tracks_temporary_update,
    get_playlist_tracks_temporary_delete,
    get_playlist_tracks_temporary_update_media_file
  }
})