...
export class UserFavoriteItems_ApiService_of_Je extends Jellyfin_Api_Services_Web {
  public async getUserFavoriteItems_Quick(userId: string, itemId: string): Promise<any> {
    if (store_server_users.server_select_kind === 'jellyfin') {
      return this.sendRequest('POST', `UserFavoriteItems/${itemId}`, {
        userId,
      })
    } else {
  ...
  public async delUserFavoriteItems_Quick(userId: string, itemId: string): Promise<any> {
    if (store_server_users.server_select_kind === 'jellyfin') {
      return this.sendRequest('DELETE', `UserFavoriteItems/${itemId}`, {
        userId,
      })
    } else {
      const response = await axios.post(
