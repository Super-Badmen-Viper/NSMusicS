...
} from "../services_web/page_lists/home_lists/index_service";
...
} from "../services_normal/media_library_scanning/index_service";
...
} from "../services_web/page_lists/artist_lists/index_service";
import {
    Album_Lists_ApiWebService_of_ND
} from "../services_web/page_lists/album_lists/index_service";
import {
    Media_Lists_ApiWebService_of_ND
} from "../services_web/page_lists/song_lists/index_service";
import {
    Playlists_ApiService_of_ND
} from "../services_normal/playlists/index_service";
import {
    Album$Medias_Lists_ApiService_of_ND
} from "../services_normal/album$songs_lists/index_service";
import {
    Browsing_ApiService_of_ND
} from "../services_normal/browsing/index_service";
...
export class Get_Subsonic_Temp_Data_To_LocalSqlite{
    ...
    private updatePlaylistMediaFilesTemporary(isAddingNewMedias) {
        const sourceArray = isAddingNewMedias ?
            store_view_media_page_info.media_Files_temporary : store_playlist_list_info.playlist_MediaFiles_temporary;
        const targetArray = isAddingNewMedias ?
        ...
        sourceArray.forEach((song) => {
            ...
            if (index < 0) {
                const newMediaData = { ...song };
                if (isAddingNewMedias) {
                    delete newMediaData.play_id;
                }
                targetArray.push(newMediaData);
            }
        });
    }
