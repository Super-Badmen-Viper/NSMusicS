                ...
                store_view_home_page_info.home_Files_temporary_maximum_playback.push(
                    {
                        ...
                        medium_image_url: album.HasCoverArt ?
                            url + '/media/cover?access_token=' + store_server_login_info.server_accessToken + '&type=album&target_id=' + album.ID
                            :
                            error_album,
                        large_image_url: '',
                        external_url: '',
            ...
            song_list.map(async (song: any, index: number) => {
                const newsong = {
                    ...
                    medium_image_url: song.HasCoverArt ?
                        url + '/media/cover?access_token=' + store_server_login_info.server_accessToken + '&type=media&target_id=' + song.ID
                        :
                        error_album,
                }
                if(store_general_fetch_media_list._load_model === 'search') {
        ...
        if (Array.isArray(song_list) && song_list.length > 0) {
            ...
            song_list.map(async (song: any, index: number) => {
                const newsong = {
                    ...
                    rg_track_peak: 0,
                    medium_image_url: song.HasCoverArt ?
                        url + '/media/cover/path?access_token=' + store_server_login_info.server_accessToken +
                        '&type=cover&target_id=' + song.ID
                        :
                        error_album,
