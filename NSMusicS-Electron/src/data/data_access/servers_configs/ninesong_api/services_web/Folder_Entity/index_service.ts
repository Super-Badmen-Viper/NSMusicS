...
export class Folder_Entity_ApiService_of_NineSong extends NineSong_Api_Services_Web {
  ...
  public async createFolder_Entity(name: string, path: string, folder_type: number): Promise<any> {
  ...
  public async updateFolder_Entity(
    id: string,
    newName: string,
  ...
  public async getFolder_Entity_All(): Promise<any> {
    try {
      const result = await this.sendRequest('GET', 'libraries', {}, {})
