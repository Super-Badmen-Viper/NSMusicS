            ...
            const artistPromises = list.flatMap((artist_list: any) =>
                artist_list.artist.map(async (artist: any) => {
                    ...
                    const albumPromises = song_list_of_this_album.map(async (album: any) => {
                        ...
                        try {
                            if (db.prepare(`SELECT COUNT(*)
                                            FROM ${store_server_user_model.annotation}
                                            WHERE item_id = ?`).pluck().get(album.id) === 0) {
                ...
                resultArray.forEach(music => {
                    ...
                    music.artist.albums.forEach((album: any) => {
                        ...
                        if (albumData) {
                            ...
                            // annotion - album
                            try {
                                let existingRecord = db.prepare(`SELECT item_id
                                                                 FROM ${store_server_user_model.annotation}
                                                                 WHERE item_id = ?`).get(albumData.id);
                                if (existingRecord) {
