...
export class Set_LocalSqlite_MediaInfo {
      ...
      if (!existingRecord) {
        db.prepare(
          `
                    ...
                    VALUES (?, ?, ?, ?, ?)`
        ).run(this.getUniqueId(db), id, 'media_file', value ? 0 : 1, this.getCurrentDateTime())
      } else {
        db.prepare(
          `
  ...
  public Set_MediaInfo_To_PlayCount_of_Media_File_ND(
    item_id: any,
    play_count: number,
    play_date: string
  ) {
