...
export class Set_LocalSqlite_PlaylistInfo {
  ...
  public Set_PlaylistInfo_To_Update_CreatePlaylist_Local(
    name: string,
    comment: string,
    duration: number,
    song_count: number,
  ...
  public Set_PlaylistInfo_To_Update_SetPlaylist_Local(
    id: string,
    name: string,
    comment: string,
    duration: number,
    ...
    if (isElectron) {
      ...
      if (!existingRecord) {
        db.prepare(
          `
                    INSERT INTO ${store_server_user_model.playlist} (id, name, comment, duration, song_count, public,
                                                                     created_at, updated_at, path, sync, size, rules,
                                                                     evaluated_at, owner_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
        ).run(
          new_id,
          name,
    ...
    if (isElectron) {
      ...
      if (existingRecord) {
        const existingTrackIds = db
          ...
          .all(playlist_id)
          .map((row: Play_list_Track) => row.media_file_id)
