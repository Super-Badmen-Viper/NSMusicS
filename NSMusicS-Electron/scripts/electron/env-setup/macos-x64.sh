...
echo "Env setup (mac x64) — running inside a Rosetta shell (arch -x86_64 zsh)"
...
echo "Using NVM to ensure Node x64 ${NODE_VERSION:-18} is available"
export NVM_ARCH=x64
NVM_ARCH=x64 nvm install 18 --no-progress || true
nvm use 18 || true
...
echo "Env setup (mac x64) completed"
