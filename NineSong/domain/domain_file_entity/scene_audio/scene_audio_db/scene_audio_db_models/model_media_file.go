...
type MediaFileMetadata struct {
	...
	Mood       string `bson:"mood"`        // 情绪标签（欢快/悲伤/史诗等）
	Language   string `bson:"language"`    // 主要语言（ISO 639-2代码）
	Lyrics     string `bson:"lyrics"`      // LRC格式歌词文本
	...
	MBZTrackID        string    `bson:"mbz_track_id"`         // MusicBrainz曲目ID（第三方元数据标识）
	...
	AcoustID          string    `bson:"acoust_id"`            // 声纹指纹标识（AcoustID数据库）
