...
namespace Local_ThisPC_song_search_ALL
{
    public partial class Form1 : Form
    {
        ...
        #region 检查歌单文件数量
        public void Init_Song_List()
        {
            int FileCount = 0;
            DirectoryInfo directoryInfo = new DirectoryInfo(Path.GetFullPath(AppDomain.CurrentDomain.BaseDirectory + @"\SongList_Ini"));
            foreach (FileInfo fileInfo in directoryInfo.GetFiles())
            {
                if (System.IO.Path.GetExtension(fileInfo.Name) == ".ini") { 
                    FileCount++;
                    comboBox1.Items.Add(fileInfo.Name);
                }
            }
        }
        ...
        {           
            ...
            {
                listBox2.Items.Add("正在遍历"+eachDrive+"盘中。。。。。");
            ...
            for (int i = 0; i < Finds_AllSong.Length; i++)
            {
                if (Finds_AllSong[i] == null)
                {
                    if (fileDicPath != null)
                    {
                        ...
                        listBox1.Items.Add(fileDicPath + "\n");
                        this.listBox1.TopIndex = this.listBox1.Items.Count - (int)(this.listBox1.Height / this.listBox1.ItemHeight);
                        break;
                    }
                }
        ...
        {
            string temp = Path.GetFullPath(AppDomain.CurrentDomain.BaseDirectory + @"\SongList_Ini\" + comboBox1.Text.Trim().ToString());
            ...
            //写入1，说明已经导入成功
            temp = Path.GetFullPath(AppDomain.CurrentDomain.BaseDirectory + @"\SongList_MakeSure_Ini\SongList_MakeSure.ini");
            Clear_File_Info(temp);
            FS_List_Save = new FileStream(temp, FileMode.Create);
            SW_List = new StreamWriter(FS_List_Save);//无法静态
