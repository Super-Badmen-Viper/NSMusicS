...
func NewRefreshTokenUsecase(userRepository domain_system_auth.UserRepository, timeout time.Duration) domain_system_auth.RefreshTokenUsecase {
...
func (rtu *refreshTokenUsecase) GetUserByID(c context.Context, email string) (domain_system_auth.User, error) {
	ctx, cancel := context.WithTimeout(c, rtu.contextTimeout)
	defer cancel()
	return rtu.userRepository.GetByID(ctx, email)
}

func (rtu *refreshTokenUsecase) CreateAccessToken(user *domain_system_auth.User, secret string, expiry int) (accessToken string, err error) {
	return token_util.CreateAccessToken(user, secret, expiry)
}

func (rtu *refreshTokenUsecase) CreateRefreshToken(user *domain_system_auth.User, secret string, expiry int) (refreshToken string, err error) {
