package usecase_system_auth_test
...
func TestFetchByUserID(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		mockTask := domain_system_auth.Task{
			ID:     primitive.NewObjectID(),
			Title:  "Test Title",
			UserID: userObjectID,
		}

		mockListTask := make([]domain_system_auth.Task, 0)
		...
		u := usecase_system_auth.NewTaskUsecase(mockTaskRepository, time.Second*2)
